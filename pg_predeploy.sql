----------------------------------------------------
-- Amazon Aurora PostgreSQL (aws_s3, rds_superuser, PostGIS).
-- Connect to database aviation before running (create DB/user once via RDS or postgres DB).
-- STEPS:
-- 0. create aviation database (one-time; see commented block below)
-- 1. create schemas
...
-- 0. create aviation database and user (one-time, while connected to postgres as RDS master user)
-- CREATE USER aviation WITH LOGIN PASSWORD '<password>';
...
# get shp2pgsql and ogr2ogr (Aurora cannot read shp files)
sudo apt-get update
...
SELECT aws_s3.table_import_from_s3('timezone_boundaries','', '(FORMAT CSV, HEADER true)',aws_commons.create_s3_uri('src-aviation', 'DIMS/CSV/timezone_simplified.csv', 'us-west-2'));
